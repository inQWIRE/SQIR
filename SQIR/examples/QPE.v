...
Lemma vsum_0 : forall d n,
  vsum n (fun _ =>  Zero : Vector d) = Zero.
Proof.
  intros d n.
  induction n; simpl. reflexivity.
  rewrite IHn. Msimpl. reflexivity.
Qed.
...
Proof.
  intros d m n f.
  induction n; simpl. 
  rewrite vsum_0. reflexivity.
  rewrite IHn. rewrite vsum_plus. reflexivity.
Qed.

Lemma vsum_singleton : forall d n j (f : nat -> Vector d),
  (j < n)%nat -> vsum n (fun i => if i =? j then (f i) else Zero) = f j.
Proof.
  intros d n j f Hj.
  induction n; try lia.
  simpl.
  bdestruct (n =? j).
  rewrite (vsum_replace_f _ _ (fun _ : nat => Zero)).
  rewrite vsum_0. Msimpl. subst. reflexivity.
  intros i Hi. bdestructΩ (i =? j). reflexivity.
  rewrite IHn by lia.
  Msimpl. reflexivity.
Qed.
...
(* Should be added to nonzero automation *)
...
Lemma QFT_w_reverse_semantics_inverse : forall n (f : nat -> bool),
  (n > 1)%nat ->
  uc_eval (invert (QFT_w_reverse n)) × f_to_vec 0 n f = (/ √(2 ^ n) .* vsum (2^n) (fun k => Cexp (- 2 * PI * INR (funbool_to_nat n f * k) / (2 ^ n)) .* basis_vector (2^n) k)).
Proof.
  intros n f Hn.
  ...
  rewrite (vsum_replace_f _ _ (fun i => / √ (2 ^ n) .* (if i =? funbool_to_nat n f then (2 ^ n) .* basis_vector (2 ^ n) i else Zero))).
  2: { intros i Hi.
       ...
       subst.
       replace (fun x : nat => Cexp (2 * PI * (INR (funbool_to_nat n f) - INR (funbool_to_nat n f)) / 2 ^ n * INR x)) with (fun _ : nat => 1 : C).
       rewrite Csum_1 by auto. 
  ...
  rewrite sqrt_pow by lra. nonzero.
  specialize (funbool_to_nat_bound n f) as Hf.
  rewrite vsum_singleton; auto.
Qed.
...
Proof.
  ...
  reflexivity. 
  apply pow_le. lra. 
  rewrite sqrt_pow; try nonzero. lra.
  rewrite sqrt_pow; try nonzero. lra.
...
Proof.
  ...
  rewrite (vsum_replace_f _ _ (fun i => if i =? funbool_to_nat k z then (2 ^ k) .* basis_vector (2 ^ k) i else Zero)).
  specialize (funbool_to_nat_bound k z) as Hz.
  rewrite vsum_singleton by assumption.
  distribute_scale. 
  ...
  replace (fun j => Cexp (2 * PI * (θ - INR i / 2 ^ k) * INR j)) with (fun _ : nat => C1).
  rewrite Csum_1 by auto.
...
(* Taken from DeutschJozsa file. Where should this definition be? *)
Definition probability_of_outcome {n} (ψ o : Vector n) : R := 
  (Cmod ((o† × ψ) 0%nat 0%nat)) ^ 2.

(* Do we aready have this somewhere? *)
Definition WF_State {n} (ψ : Vector n) := WF_Matrix ψ /\ ψ† × ψ = I 1.

Lemma basis_vector_product_eq : forall d n,
  (n < d)%nat -> (basis_vector d n)† × basis_vector d n = I 1.
Proof.
  intros d n Hn. 
  prep_matrix_equality.
  unfold basis_vector, adjoint, Mmult, I.
  bdestruct (x =? y); bdestruct (x <? 1); simpl.
  rewrite (Csum_eq _ (fun i => if i =? n then 1 else 0)).
  apply Csum_unique.
  exists n.
  repeat split; auto.
  bdestructΩ (n =? n). reflexivity.
  intros i Hi. bdestructΩ (i =? n). reflexivity.
  apply functional_extensionality; intro i.
  bdestruct (i =? n); subst; simpl.
  bdestructΩ (y =? 0).
  lca. lca.
  rewrite (Csum_eq _ (fun _ : nat => 0)).
  apply Csum_0; auto.
  apply functional_extensionality; intro i.
  subst. bdestructΩ (y =? 0).
  rewrite andb_false_r. lca.
  rewrite (Csum_eq _ (fun _ : nat => 0)).
  apply Csum_0; auto.
  apply functional_extensionality; intro i.
  bdestruct (i =? n); simpl; try lca. 
  bdestructΩ (y =? 0); lca. 
  rewrite (Csum_eq _ (fun _ : nat => 0)).
  apply Csum_0; auto.
  apply functional_extensionality; intro i.
...
Proof.
  intros d m n Hm Hn Hneq.
  prep_matrix_equality.
  unfold basis_vector, adjoint, Mmult, Zero.
  apply Csum_0; auto. 
  intro i.
  bdestruct (i =? m); bdestructΩ (i =? n); lca.
Qed.
...
Lemma QPE_semantics_full : forall k n (c : base_ucom n) z (ψ : Vector (2 ^ n)) (δ : R),
  (n > 0)%nat -> (k > 1)%nat -> uc_well_typed c -> WF_State ψ -> 
  (-1 / 2 ^ (k + 1) < δ < 1 / 2 ^ (k + 1))%R -> (δ <> 0)%R ->
  let θ := ((INR (funbool_to_nat k z) / 2 ^ k) + δ)%R in
  (uc_eval c) × ψ = Cexp (2 * PI * θ) .* ψ ->
  probability_of_outcome (@Mmult _ _ (1 * 1) (uc_eval (QPE k n c)) (k ⨂ ∣0⟩ ⊗ ψ)) ((f_to_vec 0 k z) ⊗ ψ) >= 4 / (PI ^ 2).
Proof.
  intros k n c z ψ δ Hn Hk WT [WFM WFS] [Hδlt Hδgt] Hδnz θ Heig.
  rewrite QPE_simplify with (θ := θ) by assumption.
  ...
  Msimpl. 
  rewrite Mmult_vsum_distr_l. 
  rewrite (vsum_replace_f _ _ (fun i => if i =? funbool_to_nat k z then (Csum (fun j => Cexp (2 * PI * δ) ^ j) (2 ^ k)) .* I 1 else Zero)).
  2: { intros i Hi.
       distribute_scale.
       rewrite basis_f_to_vec.
       bdestruct (i =? funbool_to_nat k z); subst.
       ...
       rewrite basis_vector_product_neq; try assumption. lma.
       apply funbool_to_nat_bound. 
       apply not_eq_sym. assumption. }
  rewrite vsum_singleton.
  2: apply funbool_to_nat_bound.
...
(* Giving up for now - we need some lemmas about bounds on sin. 
   For the rest of the proof see the last few steps of

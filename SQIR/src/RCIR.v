...
(* Basic definitions. Using Kesha's nat_to_funbool and funbool_to_nat functions, but 
   We keep the funbool to be little endianness at f0.
   Using List.rev to help us. *)
...
(* Translating N2fb to our funbool representation. *)
Check N2fb.

Check nat_of_N.

Lemma repeat_false_allfalse :
   forall len, list_to_funbool len (rev (repeat false len)) = allfalse.
Proof.
intros.
induction len.
auto.
assert ((repeat false (S len)) = (false :: repeat false len)) by auto.
rewrite H.
rewrite repeat_cons.
rewrite rev_unit.
assert (list_to_funbool (S len) (false :: rev (repeat false len)) 
          = update (list_to_funbool len%nat (rev (repeat false len))) len false).
simpl.
assert ((len - 0) = len) by lia.
...
Lemma two_reps_same : forall len n, n < 2 ^ len
            -> nat_to_fbool len n = N2fb (N_of_nat n).
Proof.
intros.
unfold nat_to_fbool,nat_to_binlist.
induction n.
simpl.
rewrite <- (repeat_false_allfalse len).
assert ((len - 0) = len) by lia.
rewrite H0.
reflexivity.
Admitted.

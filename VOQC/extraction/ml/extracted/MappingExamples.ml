...
module LNN =
 struct
  (** val coq_LNN_is_in_graph_b : int -> int -> int -> bool **)

  let coq_LNN_is_in_graph_b dim n1 n2 =
    (||)
  ...
  (** val coq_LNN_get_path : int -> int -> int list **)

  let coq_LNN_get_path n1 n2 =
    if Nat.ltb n1 n2
...
module LNNRing =
 struct
  (** val coq_LNN_ring_is_in_graph_b : int -> int -> int -> bool **)

  let coq_LNN_ring_is_in_graph_b dim n1 n2 =
  ...
  (** val coq_LNN_ring_get_path : int -> int -> int -> int list **)

  let coq_LNN_ring_get_path dim n1 n2 =
    if Nat.ltb n1 n2
...
module Grid =
 struct
  (** val grid_is_in_graph_b : int -> int -> int -> int -> bool **)

  let grid_is_in_graph_b numRows numCols i i' =
    (||)
      ((||)
        ((||)
  ...
  (** val grid_get_path : int -> int -> int -> int list **)

  let grid_get_path numCols i1 i2 =
...
module Tenerife =
 struct
  ...
  (** val tenerife_is_in_graph_b : int -> int -> bool **)

  let tenerife_is_in_graph_b n1 n2 =
    List.exists (beq_tup (n1, n2)) tenerife_graph

  (** val tenerife_get_path : int -> int -> int list **)

  let tenerife_get_path n1 n2 =
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ ->
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ -> [])

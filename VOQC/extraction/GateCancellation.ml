...
(** val coq_Rz_commute_rule1 :
    int -> coq_RzQ_ucom_l -> (coq_RzQ_Unitary gate_app list * coq_RzQ_Unitary
    gate_list) option **)
                      ...
                      | URzQ_H ->
                        Some
                          ((List.append l1
                             (List.append ((coq_H q) :: [])
                               (List.append l3
                                 (List.append ((coq_CNOT q1 q) :: [])
                                   (List.append l5 ((coq_H q) :: [])))))), l6)
                      | _ -> None)
                   | None -> None)
             else None
           | _ -> None)
...
let coq_Rz_commute_rule3 q l =
  ...
  | Some p ->
    ...
    (match r with
     | URzQ_CNOT ->
       if (=) q q1
       then Some ((List.append l1 ((coq_CNOT q1 q2) :: [])), l2)
       else None
     | _ -> None)
  | None -> None

(** val coq_Rz_commute_rules :
    int -> (coq_RzQ_ucom_l -> (coq_RzQ_Unitary gate_app
    list * coq_RzQ_Unitary gate_list) option) list **)
...
(** val coq_Rz_cancel_rule :
    int -> Q.t -> coq_RzQ_ucom_l -> coq_RzQ_Unitary gate_app list option **)
...
(** val coq_H_cancel_rule :
    int -> coq_RzQ_ucom_l -> coq_RzQ_Unitary gate_app list option **)
...
(** val coq_CNOT_commute_rule1 :
    int -> coq_RzQ_ucom_l -> (coq_RzQ_ucom_l * coq_RzQ_ucom_l) option **)
...
(** val coq_CNOT_commute_rule2 :
    int -> int -> coq_RzQ_ucom_l -> (coq_RzQ_Unitary gate_app
    list * coq_RzQ_Unitary gate_list) option **)
...
(** val coq_CNOT_commute_rule4 :
    int -> int -> coq_RzQ_ucom_l -> (coq_RzQ_Unitary gate_app
    list * coq_RzQ_Unitary gate_app list) option **)
                   ...
                   | Some p5 ->
                     ...
                     (match r1 with
                      | URzQ_H ->
                        Some
                          ((List.append l1
                             (List.append ((coq_H q2) :: [])
                               (List.append l3
                                 (List.append ((coq_CNOT q2 q2') :: [])
                                   ((coq_H q2) :: []))))),
                          (List.append l5 l6))
                      | _ -> None)
                   | None -> None)
...
(** val coq_CNOT_commute_rules :
    int -> int -> (coq_RzQ_ucom_l -> (coq_RzQ_ucom_l * coq_RzQ_ucom_l)
    option) list **)
...
(** val coq_CNOT_cancel_rule :
    int -> int -> coq_RzQ_ucom_l -> coq_RzQ_Unitary gate_app list option **)
...
(** val propagate_Rz :
    Q.t -> coq_RzQ_ucom_l -> int -> coq_RzQ_Unitary gate_app list option **)
...
(** val propagate_H :
    coq_RzQ_ucom_l -> int -> coq_RzQ_Unitary gate_app list option **)
...
(** val propagate_CNOT :
    coq_RzQ_ucom_l -> int -> int -> coq_RzQ_Unitary gate_app list option **)
...
(** val cancel_single_qubit_gates' :
    coq_RzQ_ucom_l -> int -> coq_RzQ_Unitary gate_app list -> coq_RzQ_ucom_l **)
         ...
         (match r with
          | URzQ_H ->
            (match propagate_H t q with
             ...
             | None -> cancel_single_qubit_gates' t n' ((coq_H q) :: acc))
          | URzQ_X ->
            (match propagate_X t q with
             | Some l' -> cancel_single_qubit_gates' l' n' acc
             | None -> cancel_single_qubit_gates' t n' ((coq_X q) :: acc))
          | URzQ_Rz a ->
            (match propagate_Rz a t q with
             | Some l' -> cancel_single_qubit_gates' l' n' acc
             | None -> cancel_single_qubit_gates' t n' ((coq_Rz a q) :: acc))
          | URzQ_CNOT -> cancel_single_qubit_gates' t n' (u :: acc))
       | _ -> cancel_single_qubit_gates' t n' (u :: acc)))
    n

(** val cancel_single_qubit_gates : coq_RzQ_ucom_l -> coq_RzQ_ucom_l **)
         ...
         (match r with
          | URzQ_CNOT ->
            (match propagate_CNOT t q1 q2 with
             ...
             | None -> cancel_two_qubit_gates' t n' ((coq_CNOT q1 q2) :: acc))
          | _ -> cancel_two_qubit_gates' t n' (u :: acc))
       | _ -> cancel_two_qubit_gates' t n' (u :: acc)))
    n

(** val cancel_two_qubit_gates : coq_RzQ_ucom_l -> coq_RzQ_ucom_l **)

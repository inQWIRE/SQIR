...
class TestQiskitInterop(unittest.TestCase):
    ...
    def test_AT(self):
        ...
        c.close()
        f = (os.path.join((os.path.dirname(os.path.abspath(__file__))), "interop/tests/optimized_qasm_files/optim_tof_10.qasm")).read()
        after = circuit_from_qasm(f))
        f.close()
    ...
    def test_PF(self):
        ...
        before = circuit_from_qasm(c)
        c.close()
        f = (os.path.join((os.path.dirname(os.path.abspath(__file__))), "interop/tests/optimized_qasm_files/optim_pf2_100.qasm")).read()
        after = circuit_from_qasm(f))
        f.close()
        self.assertEqual(self.run_voqc(before), after)
    """
    ...
    def run_voqc(self, before, list_opt=None):
        new_circ = VOQC().optimize_circuit(before)
        return new_circ

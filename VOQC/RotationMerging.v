...
Fixpoint find_merge_alt' {dim} (l : RzQ_ucom_l dim) (blst : FSet.t) q smap
  ...
  | App1 (URzQ_Rz a) q' :: t => 
      ...
      then Some ([], a, q', t)
           ...
           | Some (l1, a', q'', l2) => Some (Rzq a q' :: l1, a', q'', l2)
           | _ => None
            end
...
Lemma find_merge_alt'_returns_equiv : forall {dim} (l : RzQ_ucom_l dim) qs blst q smap n acc l1 a q' l2 l1' s l2' l1'' a' q'' l2'',
  ...
  a = a' /\ q' = q'' /\ 
  (forall α, l1 ++ [Rzq α q'] ++ l2 =l= rev acc ++ l1' ++ l1'' ++ [Rzq α q''] ++ l2'' ++ l2').
Proof.
  ...
  - destruct (get_subcircuit' g qs blst n) eqn:gs; destruct p.
    ...
    + destruct (find_merge_alt' l3 blst q smap) eqn:fma; try discriminate.
      ...
      simpl; rewrite rev_append_rev, rev_app_distr, rev_involutive.
      repeat rewrite app_comm_cons; rewrite (cons_to_app _ r0).
      rewrite (app_assoc _ ([Rzq a0 n0] ++ r0)), <- (app_assoc g0), (app_assoc _ _ r0).    
...
Lemma find_merge_alt_returns_equiv : forall {dim} (l : RzQ_ucom_l dim) q l1 a q' l2 l1' a' q'' l2',
  ...
  a = a' /\ q' = q'' /\
  forall α, l1 ++ [Rzq α q'] ++ l2 =l= l1' ++ [Rzq α q''] ++ l2'.
Proof.
...
Proof.
  ...
  - destruct (negb (FSet.mem n0 blst) && FSet.equal (get_set smap n0) (FSet.add q FSet.empty)).
    ...
    destruct (find_merge' g qs blst q smap n (Rzq a n0 :: rev_append g0 acc)) eqn:fm; try discriminate.
    repeat destruct p. 
    inversion H; subst.
    apply IHn in fm.
    rewrite <- fm; simpl.
...
Lemma find_merge_move_rotation_left : forall {dim} (l : RzQ_ucom_l dim) q l1 a0 q0 l2 a,
  ...
  [Rzq a q] ++ l1 ++ [Rzq a0 q0] ++ l2 =l= [Rzq a q] ++ [Rzq a0 q] ++ l1 ++ l2.
Proof.
  ...
  { unfold uc_equiv_l, uc_equiv; simpl.
    ...
    gridify; trivial. }
  assert (eq': l1 ++ l2 =l= l1 ++ [Rzq 0 q0'] ++ l2).
  ...
  rewrite (app_assoc [Rzq a0' q]), (app_assoc _ l0).
...
Lemma find_merge_move_rotation_right : forall {dim} (l : RzQ_ucom_l dim) q l1 a0 q0 l2 a,
  ...
  uc_well_typed_l l ->
  find_merge l q = Some (l1, a0, q0, l2) ->
  [Rzq a q] ++ l1 ++ [Rzq a0 q0] ++ l2 =l= l1 ++ [Rzq a q0] ++ [Rzq a0 q0] ++ l2.
Proof.
  intros dim l q l1 a0 q0 l2 a Hq Hq0 WT fm.
  specialize (find_merge_alt_returns_Some _ _ _ _ _ _ fm) as [l1' [a0' [q0' [l2' H]]]].
  specialize (find_merge_alt_returns_equiv _ _ _ _ _ _ _ _ _ _ fm H) as [? [? eq]].
  ...
  repeat destruct p.
  inversion H; subst; clear H.
  assert (double_rotation : @uc_equiv_l dim ([Rzq a q0'] ++ [Rzq a0' q0']) [Rzq (a + a0') q0']).
  { unfold uc_equiv_l, uc_equiv; simpl.
...
Fixpoint merge_rotations_at_beginning {dim} (l : RzQ_ucom_l dim) n acc :=
  ...
  | S n' => match l with
           ...
           | App1 (URzQ_Rz a) q :: t =>
               ...
               | None => merge_rotations_at_beginning t n' (Rzq a q :: acc)
               end
...
Fixpoint merge_rotations_at_end {dim} (l : RzQ_ucom_l dim) n acc :=
  ...
  | S n' => match l with
           ...
           | App1 (URzQ_Rz a) q :: t =>
               ...
               | None => merge_rotations_at_end t n' (Rzq a q :: acc)
               end
...
Proof.
  ...
  destruct (Qeq_bool (bound (a + q0)) zero_Q); repeat constructor.
  apply respects_constraints_directed_app;
...
Proof.
  ...
  with (in_l :=  (merge_rotations_at_end
            (RotationMerging.invert (merge_rotations_at_beginning l (length l) []))
            (length (merge_rotations_at_beginning l (length l) [])) []));
    try auto. 
  - apply merge_rotations_at_end_respects_constraints
      with (in_l :=(RotationMerging.invert (merge_rotations_at_beginning l (length l) [])))
           (n := (length (merge_rotations_at_beginning l (length l) [])))
           (acc := []); try constructor; try auto. 
    + apply invert_respects_constraints
        with (in_l := (merge_rotations_at_beginning l (length l) [])); try auto.
      * apply merge_rotations_at_beginning_respects_constraints
              with (in_l := l) (n := (length l)) (acc := []); try constructor; try auto. 
Qed. 

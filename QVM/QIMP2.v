...
Module Store := FMapList.Make QvarNatType.
Module StoreFacts := FMapFacts.Facts (Store).
Definition cstore : Type := Store.t nat.
Definition empty_cstore := @Store.empty nat.
...
Definition make_value (size:nat) (b:btype) (c: option nat) :=
  do cv <- c @
      match b with Bl => Some (cv mod 2)
                | Nat => Some (cv mod 2^size)
                | FixedP => Some (cv mod 2^size)
      end.
...
Definition par_eval_cfac (smap : qvar -> nat) (bv:benv) (size:nat) (r:cstore) (b:btype) (fc:cfac) := 
   match fc with Nor x => par_eval_fc bv size r b x
        | Index x n => do v <- par_eval_fc bv size r Nat n @
                      ...
                      else make_value size b (Store.find (x,v) r)) else None
   end.

Inductive value {A:Type} := Value (x:A) | Error.

Fixpoint nat2s' (x:nat) (n:nat) (size:nat) (f:nat -> bool) :=
    match n with 0 => f
             | S m => if x mod 2 then nat2s' (x/2) m size (update f (size-n) true) else nat2s' (x/2) m size f
    end.
Definition nat2s (x:nat) (n:nat) := nat2s' x n n (fun _ => false).
...
Definition par_eval_cfac_check (smap : qvar -> nat) (bv:benv) (size:nat) (r:cstore) (b:btype) (fc:cfac) := 
   match fc with Nor x => do val <- par_eval_fc bv size r b x @ Some (Value val)
        | Index x n => do v <- par_eval_fc bv size r Nat n @
                  ...
                  do val <- make_value size b (Store.find (x,v) r) @ Some (Value val)) else Some Error
   end.

Definition par_find_var (bv:benv) (size:nat)  (r:cstore) (fc:cfac) :=
       match fc with Nor (Var x) => Some (x,0)
                  | Nor (Num x) => None
                  | Index x n => do val <- par_eval_fc bv size r Nat n @ Some (x, val)
       end.
...
Definition par_find_var_check (smap:qvar -> nat) (bv:benv) (size:nat)  (r:cstore) (fc:cfac) :=
       match fc with Nor (Var x) => Some (Value (x,0))
                  ...
                  | Index x n => do val <- par_eval_fc bv size r Nat n @
                      if val <? smap x then Some (Value (x, val)) else Some Error
       end.
...
Definition clt_circuit_left (size:nat) (f:flag) (b:btype) (vmap:(qvar*nat) -> var)
                        (x :(qvar*nat)) (y:nat) (stack temp: var) (sn:nat) :=
            if f =fl= Classic then
              Exp (init_v (get_size size b) (temp) (nat2s y (get_size size b));
                    comparator01 (get_size size b) (vmap x) (temp) (stack,S sn) (stack, sn);
                    init_v (get_size size b) (temp) (nat2s y (get_size size b)))
            else rz_comparator (vmap x) (get_size size b) (stack, sn) y.

Definition clt_circuit_right (size:nat) (f:flag) (b:btype) (vmap:(qvar*nat) -> var)
                        (x:nat) (y :(qvar*nat))  (stack temp: var) (sn:nat) :=
            if f =fl= Classic then
              Exp (init_v (get_size size b) (temp) (nat2s x (get_size size b));
                    comparator01 (get_size size b) (temp) (vmap y) (stack,S sn) (stack, sn);
                    init_v (get_size size b) (temp) (nat2s x (get_size size b)))
            else Exp (init_v (get_size size b) (temp) (nat2s x (get_size size b)));;
               ...
               match par_find_var_check smap bv (get_size size b) r y with Some (Value vy) =>
                   ...
                   do t2v <- par_eval_cfac_check smap bv size r b y @ 
                    match t1v with Value t1v' => match t2v with Value t2v' =>
                      Some (Value (None, sn, Some ((t1v' <? t2v'))))
...
Definition ceq_circuit_left (size:nat) (f:flag) (b:btype) (vmap:(qvar*nat) -> var)
            ...
            if f =fl= Classic then
              Exp (init_v (get_size size b) (temp) (nat2s y (get_size size b));
                    ...
                    (X (stack,sn));
                    init_v (get_size size b) (temp) (nat2s y (get_size size b)))
            else Exp (init_v (get_size size b) (temp) (nat2s y (get_size size b)));;
                 rz_comparator (vmap x) (get_size size b) (stack, sn) y;;
                rz_full_comparator (temp) (get_size size b) (stack, sn) (vmap x) ;;
                Exp (X (stack,sn);init_v (get_size size b) (temp) (nat2s y (get_size size b))).
...
Definition ceq_circuit_right (size:nat) (f:flag) (b:btype) (vmap:(qvar*nat) -> var)
            ...
            if f =fl= Classic then
              Exp (init_v (get_size size b) (temp) (nat2s x (get_size size b));
                ...
                rz_full_comparator (temp) (get_size size b) (stack, sn)  (vmap y);;
                rz_comparator (vmap y) (get_size size b) (stack, sn)  x ;;
                        Exp ((X (stack,sn));init_v (get_size size b) (temp) (nat2s x (get_size size b))).
...
Definition compile_cexp (size:nat) (smap : qvar -> nat) (vmap: (qvar*nat) -> var)
   ...
   match e with clt b x y => 
         ...
         | iseven x => do t1 <- type_factor bv Nat x @ if fst t1 =a= Q then None else 
                        ...
                        match t2v with Value t2v' =>
                              if ( t2v') mod 2 =? 0 then Some (Value (None, sn, Some true))
              ...
              if (fst t1 =a= Q) && (fst t2 =a= C)
               then 
                 ...
                 do exps <- Store.find vx es @
                      Some (Value (Some (nadd_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn),sn,r,
                      Store.add vx ((nadd_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn)::exps) es))
                        | _ => Some Error
                     end
              ...
              if (fst t1 =a= Q) && (fst t2 =a= C)
               then 
                   do t2v <- par_eval_cfac_check smap bv size r Nat y @
                     match t2v with Value t2v' =>
                 do exps <- Store.find vx es @
                      Some (Value (Some (nsub_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn),sn,r,
                Store.add vx ((nsub_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn)::exps) es))
                      | _ => Some Error
                     end
              else if (fst t1 =a= Q) && (fst t2 =a= Q)
               then do vyv <- par_find_var_check smap bv size r y @
              ...
              if (fst t1 =a= Q) && (fst t2 =a= C)
               then 
                   do t2v <- par_eval_cfac_check smap bv size r FixedP y @
                    match t2v with Value t2v' => 
                 do exps <- Store.find vx es @
                      Some (Value (Some (fadd_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn),sn,r,
                     Store.add vx ((fadd_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn)::exps) es))
                      | _ => Some Error
                    end
              else if (fst t1 =a= Q) && (fst t2 =a= Q)
               then do vyv <- par_find_var_check smap bv size r y @
              ...
              if (fst t1 =a= Q) && (fst t2 =a= C)
               then 
                   do t2v <- par_eval_cfac_check smap bv size r FixedP y @
                    match t2v with Value t2v' =>
                     do exps <- Store.find vx es @
                      Some (Value (Some (fsub_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn),sn,r,
                       Store.add vx ((fsub_circuit_left size f vmap vx (nat2s t2v' size) stack temp sn)::exps) es))
                     | _ => Some Error
                   end
              else if (fst t1 =a= Q) && (fst t2 =a= Q)
               then do vyv <- par_find_var_check smap bv size r y @
              ...
              if (fst t2 =a= Q) && (fst t3 =a= Q) then
                 ...
                 do vyv <- par_find_var_check smap bv size r y @
                  do vzv <- par_eval_cfac_check smap bv size r Nat z @
                     ...
                     do exps <- Store.find vx es @
                           Some (Value (Some (nmul_circuit_one size f vmap vx vy (nat2s tzv size) stack temp sn),sn,r,
                      Store.add vx ((nmul_circuit_one size f vmap vx vy (nat2s tzv size) stack temp sn)::exps) es))
                      | _ => Some Error
                     end
                       | _ => Some Error
                    end
                 ...
                 do vzv <- par_find_var_check smap bv size r z @
                  do vyv <- par_eval_cfac_check smap bv size r Nat y @
                     ...
                     do exps <- Store.find vx es @
                           Some (Value (Some (nmul_circuit_one size f vmap vx vz (nat2s tyv size) stack temp sn),sn,r,
                      Store.add vx ((nmul_circuit_one size f vmap vx vz (nat2s tyv size) stack temp sn)::exps) es))
                      | _ => Some Error
                     end
                       | _ => Some Error
                    end
                    ...
                    Some (Value (Some (Exp (init_v size (vmap vx)
                               (nat2s ((tyv * tzv)  mod 2^size) size))),sn,r,
                      Store.add vx ((Exp (init_v size (vmap vx)
                               (nat2s ((tyv * tzv) mod 2^size) size)))::exps) es))
                      | _ => Some Error
                     end
                       | _ => Some Error
                    end.
                 ...
                 do vyv <- par_find_var_check smap bv size r y @
                  do vzv <- par_eval_cfac_check smap bv size r FixedP z @
                     match vyv with Value vy => 
                       match vzv with Value tzv => 
                     do exps <- Store.find vx es @
                           Some (Value (Some (fmul_circuit_one size f vmap vx vy (nat2s tzv size) stack temp sn),sn,r,
                      Store.add vx ((fmul_circuit_one size f vmap vx vy (nat2s tzv size) stack temp sn)::exps) es))
                      | _ => Some Error
                     end
                       | _ => Some Error
                    end
                 ...
                 do vzv <- par_find_var_check smap bv size r z @
                  do vyv <- par_eval_cfac_check smap bv size r FixedP y @
                     match vzv with Value vz => 
                       match vyv with Value tyv => 
                     do exps <- Store.find vx es @
                           Some (Value (Some (fmul_circuit_one size f vmap vx vz (nat2s tyv size) stack temp sn),sn,r,
                      Store.add vx ((fmul_circuit_one size f vmap vx vz (nat2s tyv size) stack temp sn)::exps) es))
                      | _ => Some Error
                     end
                       | _ => Some Error
                    end
                ...
                do t2 <- type_factor bv (get_ct t) y @
                 if fst t2 =a= Q then
                  do vyv <- par_find_var_check smap bv size r y @
                   ...
                   match t2v with Value t2v' => 
                   do exps <- Store.find vx es @
                     Some (Value (Some (Exp (bin_xor_c (get_size size (get_ct t)) (vmap vx) (nat2s t2v' size))),sn,r,
                              Store.add vx ((Exp (bin_xor_c (get_size size (get_ct t)) (vmap vx) (nat2s t2v' size)))::exps) es))
                    | _ => Some Error
                   end
                     ...
                     do t2v <- par_eval_cfac_check smap bv size r (get_ct t) y @
                       match t1v with Value t1v' => 
                         match t2v with Value t2v' => 
                            Some (Value (None,sn, (Store.add vx (t1v' * t2v') r),es))
                          | _ => Some Error end
                        | _ => Some Error
                         end
               |_ => Some Error
          ...
          do t <- BEnv.find (fst vx) bv @
               if is_q t then
                ...
                do t1v <- Store.find vx r @
                   Some (Value (None,sn, (Store.add vx t1v r),es))
            | _ => Some Error
       end.
     ...
     do t2v' <- par_eval_cfac_check smap bv size r Nat n @
       match t2v' with Value t2v =>
         let fix trans_while (r:cstore) (sn:nat) (i:nat) : option (@value (option pexp * nat * cstore * estore)) :=
            ...
            end in trans_while (Store.add (L x,0) 0 r) sn t2v
            | _ => Some Error
                       ...
                       match vxv with Value vx => 
                         ...
                         if (is_q t') && (is_q t) then
                            ...
                            do exps <- Store.find vx es @
                             ...
                             Some (Value (Some (Exp (init_v (get_size size (get_ct t)) (vmap vx) (nat2s uv (get_size size (get_ct t))))),sn,r,
                               Store.add vx ((Exp (init_v (get_size size (get_ct t)) (vmap vx) (nat2s uv (get_size size (get_ct t)))))::exps) es))
           ...
           | ndiv x y n => do t2v <- par_eval_cfac_check smap bv size r Nat y @
                             do t3v <- par_eval_cfac_check smap bv size r Nat n @
                              do vx <- par_find_var_check smap bv size r x @
                               ...
                               Some (Value ((None,sn,Store.add vx'  (t2v' / t3v') r,es)))
           ...
           | nmod x y n => do t2v <- par_eval_cfac_check smap bv size r Nat y @
                             do t3v <- par_eval_cfac_check smap bv size r Nat n @
                              do vx <- par_find_var_check smap bv size r x @
                               match t2v with Value t2v' => match t3v with Value t3v' => match vx with Value vx' =>
                               Some (Value ((None,sn,Store.add vx' ((t2v' mod t3v')) r,es)))
                             | _ => Some Error end | _ => Some Error end | _ => Some Error end
           ...
           | nfac x n =>  do t3v <- par_eval_cfac_check smap bv size r Nat n @
                      ...
                      match t3v with Value t3v' => match vx with Value vx' =>
                               Some (Value (None,sn,Store.add vx' ( (fact ( t3v'))) r,es))
                        | _ => Some Error end | _ => Some Error end
           ...
           | ncadd x y n => do t2v <- par_eval_cfac_check smap bv size r Nat y @
                             do t3v <- par_eval_cfac_check smap bv size r Nat n @
                              do vx <- par_find_var_check smap bv size r x @
                               match t2v with Value t2v' => match t3v with Value t3v' => match vx with Value vx' =>
                               Some (Value ((None,sn,Store.add vx' ( ((( t2v') + ( t3v')) mod 2^size)) r,es)))
                             | _ => Some Error end | _ => Some Error end | _ => Some Error end
           ...
           | fndiv x y n => do t2v <- par_eval_cfac_check smap bv size r Nat y @
                             do t3v <- par_eval_cfac_check smap bv size r Nat n @
                              do vx <- par_find_var_check smap bv size r x @
                               match t2v with Value t2v' => match t3v with Value t3v' => match vx with Value vx' =>
                               Some (Value ((None,sn,Store.add vx' ( ( (((( t2v') * 2^size)
                                      / ( t3v')) mod 2^size))) r,es)))
                             | _ => Some Error end | _ => Some Error end | _ => Some Error end
...
Fixpoint init_cstore_n (r:cstore) (x:qvar) (n:nat) : cstore :=
   match n with 0 => r
          | S m => Store.add (x,m) (0) (init_cstore_n r x m)
   end.
         ...
         match lookup_fmap fmap f with None => None
            | Some (rx,p,smap,vmap',bv,r) => 
               ...
               if is_q t then 
                    ...
                    Some (Value (([((G rx',0),vmap (G rx',0))]),size,sn,Exp (init_v
                        (get_size size (get_ct t)) (vmap (G rx',0)) (nat2s vxv (get_size size (get_ct t))))))
                   | _ => Some Error
               end
        end
    end
